...
class DeviceController {
	...
	public:
		...
		/**
		 * @brief Construtor do DeviceController.
		 * 
		 * Este construtor inicializa o controlador e carrega os dispositivos do banco de dados para a lista de dispositivos gerenciados.
		 */
		DeviceController();
		...
		/**
		 * @brief Atualiza o intervalo de captura de um dispositivo.
		 * 
		 * Este método procura um dispositivo na lista de dispositivos com o nome fornecido e atualiza seu intervalo de captura.
		 * 
		 * @param name O nome do dispositivo a ser atualizado.
		 * @param interval O novo intervalo de captura.
		 */
		void updateDeviceInterval(const std::string& name, bool active, int interval);

		void updateDeviceStatus(const std::string& name, int interval, bool active);

		/**
		 * @brief Retorna a lista de dispositivos gerenciados.
		 * 
		 * Este método retorna uma referência constante para a lista de dispositivos gerenciados.
		 * 
		 * @return Uma referência constante para a lista de dispositivos.
		 */
		const std::vector<Device>& getDevices() const { return this->devices; }
